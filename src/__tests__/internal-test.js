...
describe('url matching', () => {
  ...
  const tests = [
    { type: 'exact', tests: exact, matcher: (ext, match) => urlMatch({ kind: 'exact', url: base + match}, base + ext) },
    { type: 'canonical', tests: canonical, matcher: (ext, match) => urlMatch({ kind: 'canonical', url: base + match}, base + ext) },
    { type: 'substring', tests: substring, matcher: (ext, str) => urlMatch({ kind: 'substring', substring: str }, base + ext) },
    { type: 'regex', tests: regex, matcher: (ext, pattern) => urlMatch({ kind: 'regex', pattern: pattern }, base + ext) },
    { type: 'invalid', tests: invalid, matcher: (ext, match) => urlMatch({ kind: 'invalid' }, base + match) },
  ];
